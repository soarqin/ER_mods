...
void registerMimicryEstablishmentTexParam(sol::state *state, sol::table &paramsTable) {
    ...
    auto tableLoader = [delayInit = std::move(delayInit), state](const wchar_t *tableName) -> auto {
        delayInit();
        auto indexer = std::make_unique<ParamTableIndexer<MimicryEstablishmentTexParam>>(state, tableName);
        if (!indexer->isValid()) return std::unique_ptr<ParamTableIndexer<MimicryEstablishmentTexParam>>(nullptr);
    ...
    paramsTable["MimicryEstablishmentTexParam"] = [tableLoader]() -> auto { return tableLoader(L"MimicryEstablishmentTexParam"); };
}
